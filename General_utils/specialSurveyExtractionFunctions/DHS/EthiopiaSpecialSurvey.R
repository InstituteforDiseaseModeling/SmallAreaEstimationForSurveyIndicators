...
EthiopiaSpecialSurvey<-function(RawDatafilelocation,gps.file,shape,survey.id,
                                survey.file.ir,survey.file.ch,
                                strata_vars,id_var,country,surveyYear,pathToIntermediateDataFiles){
  
  # Purpose of this function: This function takes computes survey estimates by area and group for Ethiopia
  # it also writes out an intermediate file.
  ...
  # final$year<-floor(dhs$year[1])+8 # Ethiopia specific
  final$recode<-dhs$recode[1]
  final$country<-country
  final$survey<-paste0("DHS_",surveyYear)
  
  write_csv(final, paste0(pathToIntermediateDataFiles,"Processed_",country,surveyYear,"_",mapAdmin,".csv"))
